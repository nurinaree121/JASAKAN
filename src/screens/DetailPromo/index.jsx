...
const DetailPromo = ({route}) => {
  ...
  const selectedPromo = blogData.find(c => c.id === promoId);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <FastImage
          style={styles.image}
          source={{uri: selectedPromo.image}}
          resizeMode={FastImage.resizeMode.cover}
        />

        <Text style={styles.title}>{selectedPromo.title}</Text>
        <Text style={styles.date}>{selectedPromo.uploadDate}</Text>

        {/* Deskripsi */}
        <Text style={styles.description}>{selectedPromo.description}</Text>

        <View style={styles.deskripsiBox}>
          <Text style={styles.label}>Tanggal Acara:</Text>
          <Text style={styles.value}>{selectedPromo.eventDate}</Text>

          <Text style={styles.label}>Lokasi:</Text>
          <Text style={styles.value}>{selectedPromo.location}</Text>

          <Text style={styles.label}>Kuota Peserta:</Text>
          <Text style={styles.value}>{selectedPromo.quota} orang</Text>
        </View>
      ...
      <View style={styles.bottomBar}>
        ...
        <View style={{flexDirection: 'row', gap: 5, alignItems: 'center'}}>
          ...
          <Text style={styles.info}>{selectedPromo.totalComments}</Text>
        </View>
        <View style={{flexDirection: 'row', gap: 5, alignItems: 'center'}}>
          <People color={colors.grey(0.6)} variant="Linear" size={24} />
          <Text style={styles.info}>{selectedPromo.registered}</Text>
        </View>
      </View>
    </View>
  );
